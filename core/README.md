...
Core modules implement wiki-style knowledge base with 3 sections (Decisions, Patterns, Recent Work). Architecture lives in architecture.md (ref: DL-009). Issues and Key Symbols are excluded — they accumulate stale data (ref: DL-003).
...
**WikiKnowledge dataclass**: Type-safe contract between components. Covers 3 active sections: Decisions, Patterns, Recent Work. Architecture parsed for migration reads but not emitted by write_wiki. All list fields use empty lists (never None) to simplify merge logic.
...
**Regex-based parsing**: Full markdown parser (mistune, markdown-it) would be overkill for 3 known sections. Regex pattern `## SectionName` is reliable for wiki format and faster with no dependencies.
...
- Architecture content lives exclusively in architecture.md, never in context.md (ref: DL-009)

## Section Format

Wiki context.md format (3 sections):
...
**Decisions/Patterns**: Bullet lists. Each entry is independent and deduplicated.
...
**Fixed sections vs dynamic**: Core 3 sections hardcoded in parser. Adding sections requires code change rather than configuration. Simplifies implementation but reduces flexibility.
