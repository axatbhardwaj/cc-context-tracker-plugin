...
def test_main_skips_execution(mock_stdin, mock_config, mock_analyzer, mock_detector, mock_confirm, mock_exit):
    ...
    hooks.stop.json.load = MagicMock(return_value={"transcript_path": "path", "cwd": "/tmp"})
    
    mock_config.return_value = {}
    
    # Mock analyzer
    analyzer_instance = mock_analyzer.return_value
    analyzer_instance.get_changes.return_value = [MagicMock(file_path="file1", action="M")]
